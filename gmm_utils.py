    ...
    with torch.no_grad():
        ...
        for data, label in tqdm(loader):
            ...
            if isinstance(net, nn.DataParallel):
                _, out = net.module.get_features(data)
            else:
                _, out = net.get_features(data)
...
def gmm_get_logits_given_class(gmm, embeddings, classes):
    log_probs = gmm_get_logits(gmm, embeddings)  # shape is (B, C)
    B, C = log_probs.shape
        ...
        for jitter_eps in JITTERS:
            try:
                ...
                gmm = torch.distributions.MultivariateNormal(
                    loc=classwise_mean_features.float(), covariance_matrix=(classwise_cov_features + jitter).float(),
                )
                break
